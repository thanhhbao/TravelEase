...
export default function AdminDashboard() {
  ...
  const handleRoleChange = (managedUser: ManagedUser, nextRole: UserRole) => {
    if (nextRole === managedUser.role) return;
    assignUserRole(managedUser.id, nextRole);
  };

  const handleApplicationAction = (application: HostApplication, decision: HostApplicationDecision) => {
    if (application.status !== "pending") return;
    updateHostApplication(application.id, decision);
  };
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h2 className="text-xl font-semibold text-slate-900">Quản lý thành viên</h2>
              </div>
            </div>
              ...
              <table className="min-w-full divide-y divide-slate-100 text-sm">
                <thead>
                  <tr className="text-left text-slate-500">
                    <th className="py-3 pr-6 font-medium">Thành viên</th>
                    <th className="py-3 pr-6 font-medium">Vai trò</th>
                    <th className="py-3 pr-6 font-medium">Host</th>
                    <th className="py-3 pr-6 font-medium text-right">Tin đăng</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {managedUsers.map((managedUser) => (
                    <tr key={managedUser.id} className="text-slate-700">
                      <td className="py-3 pr-6">
                        <div className="flex flex-col">
                          <span className="font-semibold text-slate-900">{managedUser.name}</span>
                          <span className="text-xs text-slate-500">{managedUser.email}</span>
                        </div>
                      </td>
                      <td className="py-3 pr-6">
                        <select
                          ...
                          onChange={(event) =>
                            handleRoleChange(managedUser, event.target.value as UserRole)
                          }
                      ...
                      <td className="py-3 pr-6">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${
                            managedUser.hostStatus === "approved"
                              ? "bg-emerald-50 text-emerald-700"
                              : managedUser.hostStatus === "pending"
                                ? "bg-blue-50 text-blue-600"
                                : managedUser.hostStatus === "rejected"
                      ...
                      <td className="py-3 text-right text-slate-600">{managedUser.totalListings}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  {application.status === "pending" && (
                    <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                      <button
                        ...
                        className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-rose-200 bg-white px-3 py-2 text-sm font-semibold text-rose-600 hover:bg-rose-50"
                      >
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h2 className="text-xl font-semibold text-slate-900">Tin đăng cần duyệt</h2>
              </div>
            </div>
            <div className="space-y-4">
